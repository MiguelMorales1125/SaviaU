...
export type AdminTriviaHistory = {
  date: string;
  avgScore: number;
  attempts: number;
};
...
async function adminFetch<T>(endpoint: string, token: string, init?: RequestInit): Promise<T> {
  ...
  if (!response.ok) {
    ...
    console.error('[adminFetch] Error:', { endpoint, status: response.status, message });
...
export const adminTriviaApi = {
  ...
  fetchUserHistory: (token: string, userId: string, days = 30) =>
    adminFetch<AdminTriviaHistory[]>(`${API_CONFIG.ENDPOINTS.ADMIN_TRIVIA_PROGRESS}/../history/${userId}?days=${days}`, token),
