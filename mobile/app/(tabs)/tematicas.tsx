...
export default function Tematicas() {
  ...
  const renderAreaCard = (area: TematicaAreaSummary) => (
    <TouchableOpacity
      key={area.id}
      style={[tematicasStyles.areaCard, { backgroundColor: '#ffffff' }]}
      onPress={() => openArea(area.id)}
      activeOpacity={0.9}
    >
      <Image source={{ uri: area.heroImage }} style={tematicasStyles.areaCardImage} />
      <View style={tematicasStyles.areaCardContent}>
        <Text style={[tematicasStyles.areaCardTitle, { fontSize: getScaledSize(17) }]}>
          {area.name}
        </Text>
        <Text style={[tematicasStyles.areaCardSummary, { fontSize: getScaledSize(14) }]} numberOfLines={3}>
          {area.summary}
        </Text>
        <View style={tematicasStyles.badgeRow}>
          <View style={[tematicasStyles.badge, { backgroundColor: area.accentColor + '20', borderColor: area.accentColor }]}>
            <Text style={[tematicasStyles.badgeText, { color: area.accentColor, fontSize: getScaledSize(11) }]}>
              {area.resourceCount} opciones
            </Text>
          </View>
          {area.keywords.slice(0, 2).map((keyword) => (
            <View key={keyword} style={[tematicasStyles.badge, { backgroundColor: '#f0f9ff', borderColor: '#0ea5e9' }]}>
              <Text style={[tematicasStyles.badgeText, { color: '#0369a1', fontSize: getScaledSize(11) }]}>
                {keyword}
              </Text>
            </View>
          ))}
